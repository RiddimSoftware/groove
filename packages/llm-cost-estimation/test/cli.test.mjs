...
describe('llm-cost-estimate CLI', () => {
  it('forecasts from --size with no Linear token and emits a table', async () => {
    ...
    await withTempUsage(records, async (path) => {
      ...
      assert.match(stdout, /cost \(USD\)/);
      assert.doesNotMatch(stdout, /low confidence/);
    });
  });

  it('--size --json emits a parseable JSON payload with the same shape as the table', async () => {
    const records = [
    ...
    await withTempUsage(records, async (path) => {
      ...
      assert.equal(payload.costUsd.n, 2);
  ...
  it('--issue with LINEAR_API_TOKEN resolves the size via Linear and forecasts at that size', async () => {
    // The fake Linear endpoint returns a constant estimate so we can assert
    // that the CLI rounds-trips it into a --size match. We point the GraphQL
    // client at a local stub URL through the package's createLinearEstimateSource
    // — but the production CLI hardcodes Linear's endpoint, so we drive this
    // end-to-end through a small mock server.
    const records = [];
    for (let i = 1; i <= 5; i++) {
      const totalTokens = i * 100;
      records.push(specRecord({
    ...
    const server = createServer((req, res) => {
      ...
      req.on('end', () => {
        ...
        // The CLI queries by identifier — assert the request is shaped the way
        // the adapter contracts demand.
    ...
    try {
      await withTempUsage(records, async (path) => {
        // Drive the CLI through a child process that points the Linear adapter
        // at our mock server. Easiest path: set the env var the adapter reads,
        // and use a wrapper script flag — but the CLI exposes no endpoint
        // flag, so we shell out via node -e with an inline adapter.
        // Instead, hit the same code paths via direct import (the CLI's
        // resolve path is covered by the wrapper test below) and assert here
        // that the LinearEstimateSource piece works with a token + endpoint.
        ...
        // And smoke-test the full --size path with the resolved value:
        const { code, stdout } = await runCli(
