 ...
 * tokens, turns, dollar cost, and — for Codex records — per-window quota
 * delta. `--json` swaps the table for the same shape as JSON.
 */
...
and returns empirical P50/P80 quantiles for tokens, turns, dollars (and quota
when Codex records are present).
...
function printTable(result) {
  ...
  console.log(formatRow('cost (USD)', result.costUsd, formatUsd));
  if (result.quota !== null && result.quota !== undefined) {
    for (const label of Object.keys(result.quota).sort()) {
      console.log(formatRow(`quota ${label}`, result.quota[label], formatPercentPoints));
    }
  }
  if (result.costUsd === null) {
    console.log();
    console.log(`(no pricing rates for "${result.model}" — cost row omitted)`);
  }
}
