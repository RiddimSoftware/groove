...
// A PricingTable stub: prices every token in the bucket breakdown at a flat
// per-token rate. With the cell() helper's records (no cache/reasoning split)
// the bucket total equals the issue's totalTokens, so dollars track tokens.
function flatRatePricingTable(ratePerToken) {
  return {
    priceFor(_model, buckets) {
      const total =
        buckets.inputUncached +
        buckets.inputCached +
        buckets.cacheCreate5m +
        buckets.cacheCreate1h +
        buckets.outputVisible +
        buckets.outputReasoning;
      return total * ratePerToken;
    },
  };
}
...
describe('forecastProjectCost', () => {
  ...
  it('reports tokens, turns, and dollars channels — and excludes quota and wall-clock', async () => {
    ...
    const project = await forecastProjectCost(issues, usage, {
      seed: 5,
      pricingTable: flatRatePricingTable(RATE),
    });
  ...
  it('drops the dollar channel when any issue model is unpriceable', async () => {
    const usage = cell('L', 'sonnet', [100, 200, 300, 400, 500], 'UNP');
    // priceFor returns null (model not in the rate table) for every observation.
    const project = await forecastProjectCost([{ size: 'L', model: 'sonnet' }], usage, {
      seed: 1,
      pricingTable: { priceFor: () => null },
    });

    assert.equal(project.dollars.priced, false);
    assert.equal(project.dollars.p50, null);
    // Tokens/turns are unaffected by the pricing gap.
    assert.equal(typeof project.tokens.p50, 'number');
  });
  ...
  it('rejects malformed inputs', async () => {
    ...
    await assert.rejects(
      () => forecastProjectCost([{ size: 'L', model: 'sonnet' }], usage, { pricingTable: 'x' }),
      /pricingTable must have a priceFor/,
    );
  });
});
