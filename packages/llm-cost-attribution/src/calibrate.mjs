...
export async function calibrateCoverage(records, options = {}) {
  ...
  // Sort cells for stable output (by size, then model).
...
async function backtestCell(cell, opts) {
  ...
  const issues = [...cell.issues.values()].sort(byIssueIdentifier);
  ...
  const trainCount = issues.length - holdoutCount;

  const trainIssues = [];
  const holdoutIssues = [];
  for (let i = 0; i < order.length; i++) {
    (i < trainCount ? trainIssues : holdoutIssues).push(issues[order[i]]);
  }
...
async function groupIntoCells(records) {
  ...
  for await (const record of iterate(records)) {
    ...
    const key = cellKeyFor(size, model);
...
/** Unambiguous, printable, collision-free key for a `{ size, model }` cell. */
function cellKeyFor(size, model) {
  return JSON.stringify([size, model]);
}

function byIssueIdentifier(a, b) {
  if (a.issueIdentifier < b.issueIdentifier) return -1;
  if (a.issueIdentifier > b.issueIdentifier) return 1;
  return 0;
}
